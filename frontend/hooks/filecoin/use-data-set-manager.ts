...
import type { StorageContextHelperResult } from '../../lib/filecoin/storage-context-helper'
import {
  createStorageContextForNewDataSet,
  createStorageContextFromDataSetId,
} from '../../lib/filecoin/storage-context-helper'
...
export function useDataSetManager({
  ...
  const checkIfDatasetExists = useCallback(async (): Promise<number | null> => {
    if (isCheckingDataSetRef.current) {
      console.debug('[DataSet] Already checking data set, skipping duplicate call')
    ...
    if (!walletAddress) {
      console.debug('[DataSet] Wallet not ready yet')
      return null
    }

    if (!synapse) {
      console.debug('[DataSet] Synapse not initialized yet')
      return null
    }
    ...
    const shouldProceed = await new Promise<boolean>((resolve) => {
      setDataSet((current) => {
        ...
        if (current.status === 'initializing') {
          console.debug('[DataSet] Already initializing')
          resolve(false)
          return current
        }
        resolve(true)
    ...
    try {
      ...
      const effectiveDataSetId = urlDataSetId ?? null
